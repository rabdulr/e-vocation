...
const runSeed = async() => {
    ...
    const guy = await users.create(new Users('Guy', 'Fieri', '321 Favor Blvd', 'Flavortown', 'CA', '90017', 'Guy', 'FLAVORTOWN'));
    ...
    const item4 = await posts.create(new Posts(eva.id, 'Create a Post','This site is cool, but i\'m so confused... someone help me make a post', 'Tech', `${eva.address}, ${eva.city}, ${eva.state}, ${eva.zip}`, new Date('2020-7-31'), new Date('2020-7-31'), 10000));

    const item5 = await posts.create(new Posts(eva.id, 'Delete a post','Ok, so now i feel really silly and am looking to remove a post. Someone help?', 'Tech', `${eva.address}, ${eva.city}, ${eva.state}, ${eva.zip}`, new Date('2020-7-31'), new Date('2020-7-31'), 10000));

    const item6 = await posts.create(new Posts(guy.id, 'Surprise Birthday Party','I am throwing a birthday party for my son and I need a clown and fireworks expert', 'Entertainment', `${guy.address}, ${guy.city}, ${guy.state}, ${guy.zip}`, new Date('2020-4-31'), new Date('2020-4-31'), 10000));

    const item7 = await posts.create(new Posts(guy.id, 'Taste Test','I need new tasters for my ULTRA-SPICY sauce after previous tasters hospitalized', 'Food', `${guy.address}, ${guy.city}, ${guy.state}, ${guy.zip}`, new Date('2020-5-11'), new Date('2020-6-11'), 10000));
