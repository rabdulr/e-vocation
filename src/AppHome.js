    ...
    return (
        <div id = 'container'>
            <main className = 'z0 columnNW'>
                ...
                { window.location.hash === `#profile/${ auth.id }` && <ProfileHome auth = { auth } bids = { bids } posts = { posts } breakpoint = { breakpoint } route = { route } setFocus = { setFocus } users = { users } /> }
                { window.location.hash === `#profile/settings/${ focus }` && <ProfileSettings auth = { auth } setAuth = { setAuth } breakpoint = { breakpoint } updateUser={updateUser} route = { route }/> }
